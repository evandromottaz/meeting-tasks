...
export async function create(req: Request, res: Response) {
	...
	const { lastInsertRowid } = db.prepare('INSERT INTO usuarios (nome) VALUES (?)').run(nome);
...
export async function getById(req: Request, res: Response) {
	...
	const usuario = db.prepare('SELECT nome FROM usuarios WHERE id = ?').get(id);
...
export async function update(req: Request, res: Response) {
	...
	const { id } = req.params;
	const nome = result.data.nome.trim();
	const info = db.prepare('UPDATE usuarios SET nome = ? WHERE id = ?').run(nome, id);
